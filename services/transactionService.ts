...
export const createOrUpdateTransanction = async(
    ...
    try {
        ...
        if(image){
            const imageUploadRespone = await uploadFileToCloudinary(image, "transactions");
            if(!imageUploadRespone.success){
                return {success: false, msg: imageUploadRespone.msg || "Failed to upload the receipt"};
            }
            console.log("TransactionData: ", transactionData);

            transactionData.image = imageUploadRespone.data;
        }
        ...
        //if(transactionData.image === undefined) transactionData.image = null;

        console.log("transactionData 2: ", transactionData);
...
const revertAndUpdateWallets = async (
   ...
   newWalletId: string
    ...
    try {
        ...
        const reverIncomeExpense: number = oldTransaction.type === 'income' ? -Number(oldTransaction.amount) : Number(oldTransaction.amount);
        const revertedWalletAmount = Number(originalWallet.amount) + reverIncomeExpense;
        ...
        if(newTransactionType === 'expense'){
            //If the user tries to convert income to Expense on the same wallet
            ...
            if(oldTransaction.walletId === newWalletId && revertedWalletAmount < newTransactionAmount){
                return {success: false, msg: 'The selected walled does not have enough balance'};
            };

            //  if user tries to add expense from a new wallet but the wallet dont have enough balance
...
export const deleteTransaction = async (
    ...
    try {
        ...
        console.log("Error deleting transaction from the wallet: ", error);
        return {success: false, msg: error.message};
    }
...
export const fetchWeeklyStats = async (
    ...
    try {
        ...
        // takes each day and creates two enties in an array
